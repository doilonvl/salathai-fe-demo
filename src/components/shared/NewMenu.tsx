...
const NewMenu: React.FC = () => {
  ...
  const categories = useMemo(() => {
    const dishCategories = Array.from(new Set(SIGNATURE_DISHES_DATA.map(d => d.category)));
    return [t('menuPage.categories.all'), ...dishCategories.map(catId => t(`menuPage.categories.${catId}` as any))];
  }, [t]);
  ...
  return (
    <section id="menu" className="py-20 bg-white transition-colors duration-700">
              ...
              <div>
                ...
                <div className="flex items-center space-x-1">
                  {[...Array(dish.spiciness)].map((_, i) => (
                    <span key={i} className={`${textAccent} text-[10px]`}>●</span>
                  ))}
                  {dish.spiciness === 0 && <span className="text-stone-300 text-[10px]">○</span>}
                </div>
              </div>
            </div>
          ))}
